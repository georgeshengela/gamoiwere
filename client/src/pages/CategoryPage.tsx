...
import { useCategories, CategoryItem, useSidebarCategories } from "@/hooks/useCategories";
...
const categories = {
  smartphones: {
    ...
    name: "სმარტფონები",
    icon: "Smartphone",
    brands: ["Apple", "Samsung", "Google", "Xiaomi", "OnePlus", "Huawei", "Sony", "Nokia"],
    colors: ["შავი", "თეთრი", "ლურჯი", "წითელი", "მწვანე", "ოქროსფერი", "ვერცხლისფერი"],
    priceRange: [300, 5000],
    features: ["5G მხარდაჭერა", "წყალგაუმტარი", "უკაბელო დატენვა", "სწრაფი დატენვა", "სახის ამოცნობა", "თითის ანაბეჭდის სკანერი"]
  },
  laptops: {
    id: "laptops",
    name: "ლეპტოპები",
    ...
    priceRange: [500, 8000],
    features: ["სენსორული ეკრანი", "გრაფიკული პროცესორი", "SSD მეხსიერება", "ბიომეტრიული ავთენტიფიკაცია"]
  },
  audio: {
    id: "audio",
    name: "აუდიო",
    icon: "Headphones",
    brands: ["Sony", "Bose", "JBL", "Apple", "Samsung", "Sennheiser", "Audio-Technica"],
  ...
  smartwatches: {
    ...
    priceRange: [100, 2000],
    features: ["პულსის მონიტორინგი", "GPS", "ფიზიკური აქტივობის თვალყურის დევნება", "ECG", "წყალგაუმტარი"]
  },
  gaming: {
    id: "gaming",
    name: "გეიმინგი",
    ...
    priceRange: [50, 3000],
    features: ["უკაბელო", "RGB განათება", "პროგრამირებადი ღილაკები", "საუკეთესო ერგონომიკა"]
  },
  tablets: {
    id: "tablets",
    name: "ტაბლეტები",
    ...
    priceRange: [200, 3000],
    features: ["სტილუსის მხარდაჭერა", "კლავიატურის მხარდაჭერა", "კოსმოსური მეხსიერება", "LTE მხარდაჭერა"]
  },
  cameras: {
    id: "cameras",
    name: "კამერები",
    ...
    priceRange: [300, 8000],
    features: ["4K ვიდეო", "Wi-Fi კავშირი", "სტაბილიზაცია", "ცვალებადი ობიექტივები", "შორი ზუმი"]
  },
  sales: {
    id: "sales",
    name: "ფასდაკლებები",
...
type SortOption = "newest" | "price-low-high" | "price-high-low" | "discount" | "popular";

// Helper function to find category name from category ID in the nested structure
const findCategoryName = (categoryId: string, categories: CategoryItem[]): string | null => {
...
const CategoryPage = () => {
  ...
  useEffect(() => {
    ...
    const generateCategoryDescription = (name: string) => {
      ...
      } else if (name.includes('მამაკაც') || name.toLowerCase().includes('men')) {
      ...
      } else if (name.includes('ბავშვ') || name.toLowerCase().includes('child') || name.toLowerCase().includes('kid')) {
    ...
    metaDescription.setAttribute('content', generateCategoryDescription(categoryName));
  ...
  const productsWithImages = apiProducts.filter(p => p && (p.MainPictureUrl || (p.Pictures && Array.isArray(p.Pictures) && p.Pictures.length > 0)));
  console.log(`Products with images: ${productsWithImages.length} out of ${apiProducts.length}`);
  
  // Let's use all available API products and transform them
  const validApiProducts = apiProducts.filter(p => !!p && !!p.Id && !!p.Title);
  console.log(`Valid API products: ${validApiProducts.length}`);
  
  // First let's log the products we're working with
  console.log(`Processing ${allProducts.length} products with price ranges:`, 
  ...
  const filteredProducts = allProducts.filter(product => {
    ...
    console.log(`Evaluating product ${product.id}: price=${product.price}, hasImage=${!!product.imageUrl}`);
  ...
  const activeFiltersCount = activeBrands.length + activeColors.length + activeFeatures.length +
    (priceRange[0] !== category.priceRange[0] || priceRange[1] !== category.priceRange[1] ? 1 : 0);
    ...
    return (
      <div className="w-full lg:w-72 flex-shrink-0">
        <div className="sticky top-24">
          ...
          {activeFiltersCount > 0 && (
            <div className="mb-4">
              <div className="flex flex-wrap gap-2">
                ...
                {(priceRange[0] !== category.priceRange[0] || priceRange[1] !== category.priceRange[1]) && (
                  <Badge 
                    variant="outline"
                    className="bg-white px-2 py-1 flex items-center gap-1 text-gray-700 border-gray-300 hover:bg-gray-50 shadow-sm"
                  >
                    ...
                    <X 
                      className="h-3 w-3 ml-1 cursor-pointer text-gray-500 hover:text-gray-700" 
                      onClick={() => setPriceRange([category.priceRange[0], category.priceRange[1]])}
                    />
                  </Badge>
                )}
              </div>
          ...
          {/* Filter Options */}
          <Accordion type="multiple" defaultValue={["price", "brands", "colors"]} className="space-y-2">
            ...
            {category.brands.length > 0 && (
              <AccordionItem value="brands" className="border border-gray-200 rounded-md overflow-hidden">
                <AccordionTrigger className="text-base font-medium px-4 py-3 bg-gray-50 text-gray-700 hover:bg-gray-100">
                  ბრენდები
                </AccordionTrigger>
            ...
            {category.colors.length > 0 && (
              <AccordionItem value="colors" className="border border-gray-200 rounded-md overflow-hidden">
                <AccordionTrigger className="text-base font-medium px-4 py-3 bg-gray-50 text-gray-700 hover:bg-gray-100">
                  ფერები
                </AccordionTrigger>
                <AccordionContent className="px-4 py-3 border-t border-gray-200">
            ...
            {category.features.length > 0 && (
              <AccordionItem value="features" className="border border-gray-200 rounded-md overflow-hidden">
                <AccordionTrigger className="text-base font-medium px-4 py-3 bg-gray-50 text-gray-700 hover:bg-gray-100">
                  მახასიათებლები
                </AccordionTrigger>
                <AccordionContent className="px-4 py-3 border-t border-gray-200">
        ...
        <div className="bg-white rounded-lg sm:rounded-xl shadow-sm border border-neutral-100 p-4 sm:p-6">
          ...
          <div className="flex items-center justify-between sm:hidden">
            <div className="flex items-center flex-1 min-w-0 pr-3">
              <div className="mr-2 bg-blue-50 text-blue-600 p-1.5 rounded-lg flex-shrink-0">
                ...
                {(() => {
                  const categoryName = (realCategoryName || category.name || "").toLowerCase();
                  
                  if (categoryName.includes('ტანსაცმელი') || categoryName.includes('ქალი') || categoryName.includes('კაცი')) {
                    return <ShoppingBag className="h-4 w-4" />;
                  } else if (categoryName.includes('სახლი') || categoryName.includes('მაღაზია')) {
                    return <Store className="h-4 w-4" />;
                  } else if (categoryName.includes('ფასდაკლება') || categoryName.includes('აქცია')) {
                    return <TagIcon className="h-4 w-4" />;
                  } else if (categoryName.includes('სილამაზე') || categoryName.includes('აქსესუარები')) {
            ...
            <div className="flex items-center">
              ...
              <div className="inline-flex items-center bg-neutral-50 px-2.5 py-1 rounded-full text-xs text-neutral-700 flex-shrink-0">
                <span className="font-medium mr-1">{paginationInfo?.totalCount?.toLocaleString() || sortedProducts.length}</span> 
                <span>ნაპოვნია</span>
              </div>
            </div>
          </div>
          ...
          <div className="hidden sm:flex sm:items-center sm:justify-between">
            ...
            <div className="inline-flex items-center bg-neutral-50 px-3 py-1.5 rounded-full text-sm text-neutral-700">
              <span className="font-medium mr-1.5">{paginationInfo?.totalCount?.toLocaleString() || sortedProducts.length}</span> 
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
          ...
          <Sheet open={filterMobileOpen} onOpenChange={setFilterMobileOpen}>
            ...
            <SheetContent side="left" className="w-[300px] sm:w-[400px] p-0 overflow-y-auto">
              ...
              <div className="divide-y divide-gray-100">
                ...
                <div className="p-4">
                  {(() => {
                    const { sidebarData, isLoading } = useSidebarCategories(categoryId);
                    
                    if (isLoading) {
                      return <div className="flex justify-center py-4"><Loader className="h-5 w-5 animate-spin text-gray-400" /></div>;
                    }
                        ...
                        {sidebarData.mainCategories.length > 0 && (
                          <div className="space-y-1.5">
                            {sidebarData.mainCategories.map((cat) => {
                              ...
                              if (categoryName.includes('ტანსაცმელი') || categoryName.includes('ქალი') || categoryName.includes('კაცი')) {
                                icon = <ShoppingBag className="h-4 w-4 mr-2 text-primary" />;
                              } else if (categoryName.includes('სახლი') || categoryName.includes('მაღაზია')) {
                                icon = <Store className="h-4 w-4 mr-2 text-gray-500" />;
                              } else if (categoryName.includes('ფასდაკლება') || categoryName.includes('აქცია')) {
                                icon = <TagIcon className="h-4 w-4 mr-2 text-gray-500" />;
                              } else if (categoryName.includes('სილამაზე') || categoryName.includes('აქსესუარები')) {
                                icon = <Sparkles className="h-4 w-4 mr-2 text-gray-500" />;
                              } else {
                                icon = <Grid3X3 className="h-4 w-4 mr-2 text-gray-400" />;
                              }
                ...
                {category.brands.length > 0 && (
                  <div className="p-4">
                    <h3 className="font-medium text-gray-900 mb-3 text-sm">ბრენდები</h3>
                ...
                {category.colors.length > 0 && (
                  <div className="p-4">
                    <h3 className="font-medium text-gray-900 mb-3 text-sm">ფერები</h3>
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                      {category.colors.map(color => (
                ...
                {category.features.length > 0 && (
                  <div className="p-4">
                    <h3 className="font-medium text-gray-900 mb-3 text-sm">მახასიათებლები</h3>
                    <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                      {category.features.map(feature => (
                ...
                {/* Price Range Filter */}
                <div className="p-4">
                  <h3 className="font-medium text-gray-900 mb-3 text-sm">ფასი</h3>
                  <div className="px-2">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-sm text-gray-700 font-medium">₾{priceRange[0]}</span>
                      <span className="text-sm text-gray-700 font-medium">₾{priceRange[1]}</span>
                    </div>
                    <Slider
          ...
          >
            ...
            <SelectContent>
              ...
              <SelectItem value="price-low-high">ფასი: დაბლიდან მაღლა</SelectItem>
              <SelectItem value="price-high-low">ფასი: მაღლიდან დაბლა</SelectItem>
        ...
        <div className="flex-1">
          {isLoading ? (
            ...
            <div className="bg-white rounded-lg shadow-sm p-8 text-center">
              ...
              <p className="text-neutral-500 mb-6">
                თქვენი ძიების შედეგებზე პროდუქტები ვერ მოიძებნა. 
                გთხოვთ, გაანულოთ ფილტრები ან სცადოთ სხვა ძიების პარამეტრები.
              </p>
