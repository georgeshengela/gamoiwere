...
import {
  ...
  ProductAttribute,
} from '@/components/ui/dynamic-product-configurator';
...
const formatApiProductData = (apiData: any) => {
    ...
    Array.isArray(item.Pictures) && item.Pictures.length > 0
      ? item.Pictures.map((pic: any, index: number) => ({
          ...
          alt:
            item.Title && index === 0
              ? item.Title
              : `${item.Title || 'Product'} - Image ${index + 1}`,
        }))
      : [
          {
            id: 1,
  ...
  if (item.Price) {
    ...
    if (item.Price.ConvertedPriceWithoutSign) {
      ...
      item.Price.ConvertedPriceList.Internal
    ) {
    ...
    if (item.Price.HasSaleDiscount && item.Price.OldPrice) {
      ...
      item.Price.MarginPrice &&
      item.Price.OriginalPrice !== item.Price.MarginPrice
    ) {
  ...
  productAttributes.forEach((attr: any) => {
    if (attr.IsConfigurator) {
      if (attr.PropertyName === 'ფერი' && attr.Value) {
        const colorId = `color-${
          attr.Vid || attr.Value.replace(/\s+/g, '-').toLowerCase()
        }`;
  ...
  return {
    ...
    description:
      ...
      `
      <p>ეს არის ${
        item.Title || 'პროდუქტის'
      } დეტალური აღწერა. პროდუქტი გამოირჩევა უმაღლესი ხარისხით და თანამედროვე დიზაინით.</p>
...
const ProductDetail = () => {
  ...
  const [originalConfigPrice, setOriginalConfigPrice] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    if (
      (window as any).getPriceForConfiguration &&
      Object.keys(selectedAttributes).length > 0
    ) {
      const priceInfo = (window as any).getPriceForConfiguration(
        selectedAttributes
      );
      if (priceInfo) {
  ...
  useEffect(() => {
    ...
    return () => {
      document.title =
        'GAMOIWERE.GE - შეკვეთეთ პროდუქტები Taobao, Trendyol, Pinduoduo და Aliexpress-დან';
    };
    ...
    const fetchProductDetails = async () => {
      try {
        ...
        const response = await fetch(
          `https://service.devmonkeys.ge/api/batchGetItemFullInfo?itemId=${productId}`
        );
        ...
        if (
          data.Result.Item.ActualWeightInfo &&
          data.Result.Item.ActualWeightInfo.Weight
        ) {
        ...
        if (data.Result.Item.IsSellAllowed !== undefined) {
          ...
          console.log(
            'Product sell status:',
            data.Result.Item.IsSellAllowed
              ? 'Available'
        ...
        if (
          data.Result.Item.Attributes &&
          Array.isArray(data.Result.Item.Attributes)
        ) {
          ...
          const checkAvailabilityForAttribute = (
            attr: any,
            ...
            const matchingItems = configuredItems.filter(
              (item: any) =>
                item.Configurators &&
            ...
            const availableItems = matchingItems.filter((item: any) => {
              ...
              for (const [selectedPid, selectedVid] of Object.entries(
                currentSelection
              )) {
                if (selectedPid !== attr.Pid) {
                  // Don't check against the attribute we're testing
                  const hasMatchingConfig = itemConfigs.some(
                    (config: any) =>
          ...
          processedAttributes.forEach((attr) => {
            ...
            if (!attr.isAvailable) {
              console.log(
                `Configuration ${attr.PropertyName}: ${attr.Value} is completely out of stock`
              );
            }
          });

          // Store the availability checker function for dynamic updates
          (window as any).checkAttributeAvailability =
            checkAvailabilityForAttribute;
          ...
          const allPrices = configuredItems
            .filter(
              (item: any) => item.Price?.ConvertedPriceList?.Internal?.Price
            )
          ...
          const getPriceForConfiguration = (
            ...
            const matchingItem = configuredItems.find((item: any) => {
              ...
              return (
                Object.entries(selectedConfig).every(([pid, vid]) => {
                  return item.Configurators.some(
                    (config: any) => config.Pid === pid && config.Vid === vid
                  );
                }) &&
            ...
            if (matchingItem && matchingItem.Price) {
              const currentPrice =
                matchingItem.Price.ConvertedPriceList?.Internal?.Price || 0;
              return {
                price: currentPrice,
                currency:
                  matchingItem.Price.ConvertedPriceList?.Internal?.Sign || '₾',
                inStock: matchingItem.Quantity > 0,
                originalPrice:
                  highestPrice > currentPrice ? highestPrice : null,
                discountPercentage:
          ...
          const configuratorAttributes = processedAttributes.filter(
            (attr: ProductAttribute) => attr.IsConfigurator
          );
        ...
        if (
          formattedProduct.variations.colors &&
          formattedProduct.variations.colors.length > 0
        ) {
          const availableColor = formattedProduct.variations.colors.find(
            (c: any) => c.available
          );
          setSelectedColor(
            availableColor
              ? availableColor.id
              : formattedProduct.variations.colors[0].id
          );
        }

        // Set default selected size to the first available size
        if (
          formattedProduct.variations.sizes &&
          formattedProduct.variations.sizes.length > 0
        ) {
          const availableSize = formattedProduct.variations.sizes.find(
            (s: any) => s.available
          );
          setSelectedSize(
            availableSize
              ? availableSize.id
              : formattedProduct.variations.sizes[0].id
          );
        }
        ...
        const productId = params?.id
          ? parseInt(params.id.replace(/^tr-/, ''), 10)
          : 0;
  ...
  const handleAddToCart = () => {
    ...
    if (
      ...
      productAttributes.some((attr) => attr.IsConfigurator)
    ) {
      ...
      if (product.variations?.hasColors && selectedColor) {
        const selectedColorObj = product.variations.colors.find(
      ...
      if (product.variations?.hasSizes && selectedSize) {
        const selectedSizeObj = product.variations.sizes.find(
  ...
  return (
    <main className="bg-gray-50">
      ...
      <h1 className="sr-only">
        {translatedTitle || product.name} - შეუკვეთე გამოიწერე.GE-დან
      </h1>
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <SocialMetaTags
          ...
          description={`${translatedTitle || product.name} - ${
            configurationPrice?.price || product.price
          }${
            product.sign
          } - GAMOIWERE.GE-ზე შეიძინეთ უმაღლესი ხარისხის პროდუქტები. უფასო მიწოდება, სწრაფი გაფორმება, საიმედო მომსახურება.`}
          imageUrl={
            ...
            <div className="absolute inset-0 bg-gray-100 bg-opacity-70 z-10 flex flex-col items-center justify-center p-4">
              <div className="bg-white p-4 sm:p-6 rounded-lg shadow-lg text-center max-w-sm sm:max-w-md mx-auto">
                ...
                <p className="text-sm sm:text-base text-gray-600 mb-3 sm:mb-4">
                  სამწუხაროდ, ეს პროდუქტი ამჟამად არ იყიდება. გთხოვთ, შეამოწმოთ
                  მოგვიანებით ან დაათვალიეროთ მსგავსი პროდუქტები.
                </p>
                <Button
                  asChild
                      ...
                      if (Object.keys(selectedAttributes).length > 0) {
                        ...
                        for (const attrPid of Object.keys(selectedAttributes)) {
                          ...
                          const matchingAttr = productAttributes.find(
                            (attr) =>
                              attr.Pid === attrPid &&
                              attr.Vid === selectedAttrVid &&
                              (attr.ImageUrl || attr.MiniImageUrl)
                          );
                          // Return the image URL if found
                          if (matchingAttr) {
                            return (
                              matchingAttr.ImageUrl || matchingAttr.MiniImageUrl
                            );
                          }
                        }
                      }
            ...
            <div className="flex flex-col h-full justify-between">
                ...
                <div className="p-3 sm:p-4 md:p-6 border-b border-slate-100">
                  <div className="flex items-center space-x-2 sm:space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs sm:text-xs md:text-sm text-slate-500">
                        სახელი და მახასიათებლები
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 md:gap-6">
                      ...
                      <div className="flex items-center">
                        {[...Array(5)].map((_, i) => {
                          const apiRating =
                            product?.apiData?.Result?.Item?.FeaturedValues?.find(
                              (feature: any) => feature.Name === 'rating'
                            )?.Value;

                          const ratingValue = apiRating
                            ? parseFloat(apiRating)
                            : productRating;
                          ...
                          return (
                            <div className="ml-1.5 sm:ml-2 flex items-center gap-1">
                              <span className="text-xs sm:text-sm font-medium text-slate-700">
                                {apiRating
                                  ? parseFloat(apiRating).toFixed(1)
                  ...
                  <div className="p-3 sm:p-4 md:p-6 border-b border-slate-100">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center bg-slate-100 rounded-lg px-2 sm:px-3 py-1.5 sm:py-2">
                        ...
                        <span className="text-xs sm:text-sm font-medium text-slate-700">
                          {product.apiData?.Result?.Item?.SalesCount ||
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 sm:gap-5">
                      ...
                      <div className="flex-1">
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <h2 className="text-[25px] sm:text-4xl font-bold text-gray-800">
                                {configurationPrice
                                  ? `${
                                      configurationPrice.currency
                                    }${configurationPrice.price.toFixed(2)}`
                                  : `₾${(configPrice || product.price).toFixed(
                                      2
                                    )}`}
                              </h2>

                              {(configurationPrice?.discountPercentage ||
                          ...
                          {(configurationPrice?.originalPrice ||
                            configOldPrice ||
                            product.oldPrice) && (
                              ...
                              <div className="flex items-center px-2 py-1 bg-green-50 rounded-lg">
                                ...
                                <span className="text-sm text-green-600 font-medium">
                                  {(() => {
                                    ...
                                    return `დაზოგეთ ₾${discount.toFixed(
                                      0
                                    )} (${percentage}%)`;
                                  })()}
                                </span>
                              ...
                              <div className="flex items-center justify-between">
                                ...
                                <Button
                                  ...
                                  onClick={() => {
                                    ...
                                    window.location.href = `/brand/${product.vendor.name
                                      .toLowerCase()
                                      .replace(/\s+/g, '-')}`;
                                  }}
                                >
              ...
              <div className="mb-5 sm:mb-8">
                ...
                productAttributes.some((attr) => attr.IsConfigurator) ? (
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs md:text-sm text-slate-500">
                              აირჩიეთ თქვენთვის შესაფერისი ვარიანტი
                            </p>
                          </div>
                        </div>
                        <button
                        ...
                        onAttributeChange={(propertyId, valueId) => {
                          ...
                          if (product?.apiData?.Result?.Item) {
                            ...
                            if (sizeValue === '70x77') {
                              ...
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice + 5.2 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price + 4.21
                                } ₾`
                              );
                            } else if (sizeValue === '70x38') {
                              // Medium size - medium price
                              setConfigPrice(product.price + 2.7);
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice + 3.1 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price + 2.7
                              ...
                              setConfigPrice(product.price + 1.2);
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice + 1.5 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price + 1.2
                              ...
                              setConfigPrice(product.price - 2.3);
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice - 1.8 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price - 2.3
                              ...
                              setConfigPrice(product.price + 1.8);
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice + 2.2 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price + 1.8
                              ...
                              // Other size
                              setConfigPrice(product.price + 3.1);
                              setConfigOldPrice(
                                product.oldPrice ? product.oldPrice + 3.5 : null
                              );
                              console.log(
                                `Updated price for ${sizeValue} configuration: ${
                                  product.price + 3.1
                              ...
                              console.log(
                                `Reset to original price: ${product.price} ₾`
                              );
                            }
                          }
                        }}
                  ...
                  (() => {
                    ...
                    const hasStandardOnlyColors =
                      product.variations?.hasColors &&
                      product.variations.colors.length === 1 &&
                      (product.variations.colors[0].name
                        .toLowerCase()
                        .includes('სტანდარტული') ||
                        product.variations.colors[0].name
                          .toLowerCase()
                          .includes('standard') ||
                        product.variations.colors[0].name
                          .toLowerCase()
                          .includes('ჩვეულებრივი') ||
                        product.variations.colors[0].name
                          .toLowerCase()
                          .includes('default'));

                    const hasStandardOnlySizes =
                      product.variations?.hasSizes &&
                      product.variations.sizes.length === 1 &&
                      (product.variations.sizes[0].name
                        .toLowerCase()
                        .includes('სტანდარტული') ||
                        product.variations.sizes[0].name
                          .toLowerCase()
                    ...
                    const isStandardOnly =
                      (hasStandardOnlyColors &&
                        (!product.variations.hasSizes ||
                          hasStandardOnlySizes)) ||
                      (hasStandardOnlySizes &&
                        (!product.variations.hasColors ||
                          hasStandardOnlyColors)) ||
                      (!product.variations?.hasColors &&
                              ...
                              <div>
                                ...
                                <div className="flex items-center space-x-2">
                                  ...
                                  <div className="text-xs text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full">
                                    {selectedColor
                                      ? product.variations.colors.find(
                                          (c: any) => c.id === selectedColor
                                        )?.name || ''
                                      : 'აირჩიეთ'}
                                  </div>
              ...
              <div className="mb-4 sm:mb-6 md:mb-8">
                ...
                <div className="flex flex-col gap-2 sm:gap-3 mt-6 sm:mt-8">
                  ...
                  <Button
                    className={`gap-2 h-11 sm:h-12 text-sm sm:text-base w-full transition-transform shadow-md ${
                      !product.isSellAllowed
                  ...
                  <div className="flex gap-2 sm:gap-3">
                    <Button
                      variant="secondary"
                      className={`gap-1.5 sm:gap-2 h-11 sm:h-12 text-sm sm:text-base flex-1 transition-transform shadow-sm ${
                        !product.isSellAllowed
                          ? 'bg-gray-200 text-gray-400 hover:bg-gray-200 border-gray-300 cursor-not-allowed'
                          : ''
                      }`}
                  ...
                  <div className="space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        style={{
                          background: `linear-gradient(to right, #5b38ed 0%, #5b38ed ${
                            ((selectedMonths - 3) / 33) * 100
                          }%, #e5e7eb ${
                            ((selectedMonths - 3) / 33) * 100
                          }%, #e5e7eb 100%)`,
                        }}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        style={{
                          background: `linear-gradient(to right, #5b38ed 0%, #5b38ed ${
                            (firstPayment / Math.floor(product.price * 0.5)) *
                            100
                          }%, #e5e7eb ${
                            (firstPayment / Math.floor(product.price * 0.5)) *
                            100
        ...
        <div className="mb-6">
          <Tabs defaultValue="reviews" className="w-full">
            ...
            <TabsContent
              value="description"
              className="bg-white sm:rounded-lg p-4 sm:p-6 sm:shadow-sm mt-0 sm:mt-2"
            >
              {renderDescription()}
            </TabsContent>
            <TabsContent
                ...
                {product.specifications.length > 8 && (
                  <div className="text-center pt-2">
                    <span className="text-sm text-gray-500">
                      +{product.specifications.length - 8} მეტი მახასიათებელი
                    </span>
                  </div>
                )}
              </div>
            ...
            >
              {(() => {
                ...
                const providerReviews =
                  product?.apiData?.Result?.ProviderReviews?.Content || [];
                const displayedReviews = showAllReviews
                  ? providerReviews
                  : providerReviews.slice(0, 5);
                  ...
                  return (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-500 text-sm">
                        ამ პროდუქტს ჯერ არ აქვს მომხმარებელთა შეფასებები
                      </p>
                    </div>
                  );
                }
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-start justify-between mb-3">
                                <div className="flex items-center gap-3">
                                  <div className="w-10 h-10 bg-[#5b38ed] rounded-full flex items-center justify-center text-white font-medium text-sm">
                                    {review.UserNick
                                      ? review.UserNick.charAt(0).toUpperCase()
                                      : 'U'}
                                  </div>
                                  <div>
                              ...
                              <div className="text-gray-700 text-sm leading-relaxed">
                                {review.Content ||
                                  'შეფასება არ არის მითითებული'}
                              </div>

                              {review.Images && review.Images.length > 0 && (
                                <div className="mt-3 flex gap-2">
                                  {review.Images.slice(0, 2).map(
                          ...
                          <div className="relative pt-4">
                            ...
                            >
                              ...
                              <span className="text-white/80 text-sm">
                                ({providerReviews.length - 5} დარჩენილი)
                              </span>
                            </button>
                          </div>
                        </div>
        ...
        <div className="mb-12">
          {(() => {
            ...
            const recommendedItems =
              product?.apiData?.Result?.RecommendedItems?.Content || [];
            ...
            if (recommendedItems && recommendedItems.length > 0) {
              ...
              const recommendedApiProducts = recommendedItems.map(
                (item: any) => {
                  // Get the rating and reviews from FeaturedValues
                  const rating = item.FeaturedValues?.find(
                    (feature: any) => feature.Name === 'rating'
                  )?.Value;
                  const reviews = item.FeaturedValues?.find(
                    (feature: any) => feature.Name === 'reviews'
                  )?.Value;
                  const favCount = item.FeaturedValues?.find(
                    (feature: any) => feature.Name === 'favCount'
                  )?.Value;

                  // Get price information
                  const price =
                    item.Price?.ConvertedPriceList?.DisplayedMoneys?.[0]
                      ?.Price ||
                  ...
                  console.log(
                    `Processing recommended product: ${item.Id} - ${item.Title}`
                  );
                  console.log(
                    `  Price: ${price}, OldPrice: ${oldPrice}, Discount: ${discountPercentage}%`
                  );
              ...
              console.log(
                'No recommended items found in Content array, using fallback products'
              );
              ...
              return (
                <ProductGrid
                  title="მსგავსი პროდუქტები"
                  products={[...popularProducts, ...recommendedProducts].slice(
                    0,
                    6
                  )}
                  viewAllLink="/products"
