...
function DeliveryTrackingCardComponent() {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case "ORDERED": return "შეკვეთილია";
      case "RECEIVED_CHINA": return "მიღებულია ჩინეთში";
      case "SENT_TBILISI": return "გაგზავნილია თბილისში";
      case "DELIVERED_TBILISI": return "ჩაბარებულია";
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "RECEIVED_CHINA": return "bg-yellow-100 text-yellow-700";
      case "SENT_TBILISI": return "bg-purple-100 text-purple-700";
      case "DELIVERED_TBILISI": return "bg-green-100 text-green-700";
      default: return "bg-gray-100 text-gray-700";
    }
  };

  const latestTracking = Array.isArray(deliveryTrackings) && deliveryTrackings.length > 0 
    ? deliveryTrackings[0] 
    : null;
      ...
      <CardContent className="p-5 flex-1 flex flex-col">
        {isLoading ? (
          ...
          <div className="flex-1 space-y-3">
            <div className="flex justify-between items-start">
              ...
              <Badge className={`${getStatusColor(latestTracking.delivery_status)} border-0 font-normal`}>
            ...
            {latestTracking.tracking_number && (
              <div className="bg-gray-50 p-2 rounded">
                ...
                <p className="font-mono text-sm text-gray-700">{latestTracking.tracking_number}</p>
              </div>
            )}
          </div>
        ) : (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">მიწოდების ინფორმაცია არ მოიძებნა</p>
            </div>
          </div>
        )}
        ...
        <Button 
          ...
          onClick={() => {
            ...
            const deliveryTab = document.querySelector('[data-tab="delivery"]') as HTMLElement;
...
function DeliveryTrackingTab() {
  ...
  const { data: deliveryTrackings, isLoading, error } = useQuery({
    queryKey: ['/api/user/delivery-tracking'],
    enabled: !!user,
  });
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case "RECEIVED_CHINA": return <Package className="h-4 w-4" />;
      case "SENT_TBILISI": return <Plane className="h-4 w-4" />;
      case "DELIVERED_TBILISI": return <CheckCircle className="h-4 w-4" />;
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case "ORDERED": return "პროდუქტი შეკვეთილია";
      case "RECEIVED_CHINA": return "პროდუქტი მიღებულია ჩინეთში";
      case "SENT_TBILISI": return "პროდუქტი გაგზავნილია თბილისში";
      case "DELIVERED_TBILISI": return "პროდუქტი ჩაბარებულია თბილისში";
      default: return "უცნობი სტატუსი";
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {!deliveryTrackings || !Array.isArray(deliveryTrackings) || deliveryTrackings.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-8">
          <div className="text-center py-8">
            <Navigation className="h-16 w-16 mx-auto mb-4 text-gray-300" />
            <h3 className="text-lg font-medium text-gray-600 mb-2">მიწოდების ინფორმაცია არ მოიძებნა</h3>
            <p className="text-gray-500">თქვენი შეკვეთებისთვის მიწოდების ინფორმაცია ჯერ არ არის დამატებული</p>
          </div>
        </div>
      ) : (
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-50">
                <TableHead className="font-semibold text-gray-700">შეკვეთის ID</TableHead>
                <TableHead className="font-semibold text-gray-700">სტატუსი</TableHead>
                <TableHead className="font-semibold text-gray-700">ტრეკინგ კოდი</TableHead>
                <TableHead className="font-semibold text-gray-700">წონა</TableHead>
                <TableHead className="font-semibold text-gray-700">ტრანსპორტ. ღირებულება</TableHead>
                <TableHead className="font-semibold text-gray-700">ბოლო განახლება</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {deliveryTrackings.map((tracking: any) => (
                <TableRow key={tracking.id} className="hover:bg-gray-50 transition-colors">
                  <TableCell className="font-medium text-[#5b38ed]">
                    #{tracking.order_id}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className={`p-1.5 rounded-full ${getStatusColor(tracking.delivery_status)}`}>
                        {getStatusIcon(tracking.delivery_status)}
                      </div>
                      <div>
                        <p className="font-medium text-sm">{getStatusText(tracking.delivery_status)}</p>
                        <p className="text-xs text-gray-500">{formatDate(tracking.updated_at)}</p>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
...
function NotificationsTab() {
  const { notifications, isLoading, markAsRead, markAllAsRead, clearReadNotifications, unreadCount } = useNotifications();
      ...
      {notifications.length === 0 ? (
        <Card className="border border-gray-200 shadow-sm">
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">შეტყობინებები არ არის</h3>
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        <span className={`font-medium ${notification.isRead ? 'text-gray-900' : 'text-gray-900 font-semibold'}`}>
                          {notification.title}
                        </span>
                    ...
                    <TableCell>
                      <p className={`text-sm ${notification.isRead ? 'text-gray-600' : 'text-gray-800'} line-clamp-2`}>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200">
              <div className="flex items-center text-sm text-gray-600">
                <span>
                  {startIndex + 1}-{Math.min(endIndex, notifications.length)} of {notifications.length} შეტყობინება
                </span>
              </div>
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    console.log('User pending transportation fees:', user?.pending_transportation_fees);
          ...
          <div className="flex flex-col items-center justify-center min-h-[60vh]">
            ...
            <div className="text-center max-w-md mx-auto">
              ...
              <p className="text-lg text-gray-600 dark:text-gray-300 mb-8 leading-relaxed">
                პროფილის სანახავად და თქვენი შეკვეთების მართვისთვის გაიარეთ ავტორიზაცია სისტემაში
              </p>
              
              {/* Action Buttons */}
              <div className="space-y-4">
                <Link to="/login" className="block">
                  <Button size="lg" className="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-medium py-3 px-8 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200">
                ...
                <Link to="/register" className="block">
                  <Button variant="outline" size="lg" className="w-full border-2 border-gray-300 dark:border-gray-600 hover:border-purple-500 dark:hover:border-purple-400 font-medium py-3 px-8 rounded-lg transition-all duration-200">
            ...
            <div className="mt-16 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
              <Card className="text-center p-6 border-0 shadow-md hover:shadow-lg transition-shadow duration-200">
                <CardContent className="pt-4">
                  ...
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-2">შეკვეთების მართვა</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">თვალყური ადევნეთ თქვენს შეკვეთებს და მიწოდების სტატუსს</p>
                </CardContent>
              </Card>
              ...
              <Card className="text-center p-6 border-0 shadow-md hover:shadow-lg transition-shadow duration-200">
                <CardContent className="pt-4">
                  ...
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-2">რჩეული პროდუქტები</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">შეინახეთ და მართეთ თქვენი საყვარელი პროდუქტები</p>
                </CardContent>
              </Card>
              ...
              <Card className="text-center p-6 border-0 shadow-md hover:shadow-lg transition-shadow duration-200">
                <CardContent className="pt-4">
                  ...
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-2">ბალანსის მართვა</h3>
                  <p className="text-sm text-gray-600 dark:text-gray-300">შეავსეთ ბალანსი და გაუმჯობესეთ შოპინგის გამოცდილება</p>
                </CardContent>
              </Card>
            </div>
          </div>
  ...
  return (
    <div className="container mx-auto px-4 py-10">
      {/* SEO H1 Heading */}
      <h1 className="sr-only">მომხმარებლის პროფილი - პირადი ინფორმაცია და პარამეტრები</h1>
        ...
        <div className="w-full md:w-72 space-y-4">
          <Card className="overflow-hidden border-gray-200 shadow-sm">
            <CardHeader className="text-center bg-[#f8f7ff] border-b border-gray-100 pb-6">
              ...
              <CardTitle className="mt-2 text-gray-800">{user.username}</CardTitle>
              ...
              <div className="mt-3 flex flex-col gap-2 items-center">
                <Badge className="bg-[#f1eeff] text-[#5b38ed] border-0 font-light px-3 py-1 text-xs">
                  {user.role === 'admin' ? 'ადმინისტრატორი' : 
                   user.role === 'moderator' ? 'მოდერატორი' : 
                ...
                <Badge className={`${
                  ...
                  {user.verification_status === 'verified' ? 'ვერიფიცირებული' : 'არავერიფიცირებული'}
                </Badge>
              </div>
            </CardHeader>
            ...
            <div className="space-y-8">
              ...
              {user && user.verification_status !== 'verified' && (
                <Alert className="border-orange-200 bg-orange-50">
                  ...
                  <AlertTitle className="text-orange-800">მობილური ნომრის ვერიფიკაცია</AlertTitle>
                  <AlertDescription className="text-orange-700">
                    შეკვეთების განთავსებისთვის საჭიროა მობილური ნომრის ვერიფიკაცია.
                    <Button 
                  ...
                  <CardContent className="p-4 flex flex-col h-full">
                    ...
                    <div className="space-y-2 flex-1">
                      <div className={`text-2xl font-bold ${
                        getEffectiveBalance() >= 0 ? 'text-green-600' : 'text-red-600'
                      }`}>
                      ...
                      {(user?.pending_transportation_fees || 0) > 0 && (
                        <p className="text-sm text-red-500">
                          აქედან მიწოდების ღირებულება: -{((user?.pending_transportation_fees || 0) / 100).toFixed(2)} ₾
                        </p>
                      )}
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {Array.isArray(orders) && orders.length > 0 && (
                        <Badge variant="outline" className="text-xs">
                          {orders.filter((o: any) => o.status === "PENDING" || o.status === "PROCESSING").length} აქტიური
                        </Badge>
                      )}
                    </div>
                    ...
                    <div className="space-y-2 flex-1">
                      ...
                      {Array.isArray(orders) && orders.length > 0 && (
                        <div className="text-sm text-gray-600">
                          {orders.filter((o: any) => o.status === "DELIVERED").length} დასრულებული
                        </div>
                      )}
                    </div>
                  ...
                  <CardContent className="p-4 flex flex-col h-full">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-medium text-gray-900">მიწოდების სტატუსი</h3>
                      </div>
                    ...
                    <div className="flex-1">
                      {Array.isArray(deliveryTrackings) && deliveryTrackings.length > 0 ? (
                        ...
                        <div className="space-y-2 text-center py-4">
                          ...
                          <div className="text-sm text-gray-500">აქტიური მიწოდებები არ არის</div>
                        </div>
                      )}
                    </div>
                ...
                {Array.isArray(orders) && orders.length > 0 ? (
                  <div className="grid grid-cols-1 gap-5">
                    {orders
                      ...
                      .slice(0, 3)
                      .map((order: any) => (
                      <Card key={order.id} className="w-full overflow-hidden border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                        <CardContent className="p-0">
                          <div className="flex flex-col divide-y divide-gray-100">
                            <div className="p-4 flex items-center justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <p className="font-medium text-sm">შეკვეთა #{order.orderNumber}</p>
                                  <p className="text-xs text-gray-500">{new Date(order.createdAt).toLocaleDateString("ka-GE", {
                                    year: "numeric",
                                    month: "long",
                                    day: "numeric",
                                  })}</p>
                            ...
                            <div className="px-4 py-3 flex flex-wrap gap-x-8 gap-y-2 bg-gray-50">
                              <div>
                                <p className="text-xs text-gray-500">ნივთები</p>
                                <p className="font-medium text-sm">{order.items ? order.items.length : order.itemCount} ერთეული</p>
                              </div>
                              <div>
                                <p className="text-xs text-gray-500">ჯამი</p>
                                <p className="font-medium text-sm">{(order.totalAmount / 100).toFixed(2)} ₾</p>
                              </div>
                  ...
                  <Card className="text-center p-8 border border-gray-200 bg-white">
                    ...
                    <h4 className="text-lg font-medium text-gray-800 mb-2">შეკვეთები არ გაქვთ</h4>
                    <p className="text-gray-500 text-sm max-w-xs mx-auto">
                      დაიწყეთ შოპინგი თქვენი პირველი შეკვეთისთვის ჩვენს პლატფორმაზე
                    </p>
                    <Link to="/products">
                    ...
                    {deliveryTrackings.map((tracking: any) => {
                      ...
                      const getStatusColor = (status: string) => {
                        switch (status) {
                          ...
                          case "RECEIVED_CHINA": return "bg-yellow-50 text-yellow-700 border border-yellow-200";
                          case "SENT_TBILISI": return "bg-purple-50 text-purple-700 border border-purple-200";
                          case "DELIVERED_TBILISI": return "bg-green-50 text-green-700 border border-green-200";
                      ...
                      const getStatusIcon = (status: string) => {
                        switch (status) {
                          case "ORDERED": return "📦";
                          case "RECEIVED_CHINA": return "🏭";
                          case "SENT_TBILISI": return "✈️";
                          case "DELIVERED_TBILISI": return "✅";
                          default: return "📋";
                        }
                      };

                      return (
                        <Card key={tracking.id} className="border border-gray-100 hover:border-gray-200 transition-all duration-200 hover:shadow-sm">
                          <CardContent className="p-5">
                            <div className="flex items-center gap-4">
                              {/* Status Icon */}
                              <div className="w-10 h-10 bg-white border border-gray-200 rounded-lg flex items-center justify-center shrink-0">
                                <span className="text-lg">{getStatusIcon(tracking.delivery_status)}</span>
                              </div>
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <Badge className={`${getStatusColor(tracking.delivery_status)} font-medium px-2 py-1 text-xs rounded-full shrink-0 ml-2`}>
                                ...
                                {tracking.tracking_number && (
                                  <div className="flex items-center gap-2 mb-3">
                                    <span className="text-xs text-gray-500">ტრეკინგ:</span>
                                    ...
                                    <Button 
                                      ...
                                      onClick={() => {
                                        ...
                                        toast({
                                          title: "კოპირებულია",
                                          description: "ტრეკინგ კოდი კოპირებულია",
                                        });
                                      }}
                                ...
                                <div className="flex items-center gap-4 text-sm">
                                  {tracking.product_weight && (
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-gray-600">{tracking.product_weight} კგ</span>
                                    </div>
                                  )}
                                  {tracking.transportation_price && (
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-gray-600">{(tracking.transportation_price / 100).toFixed(2)}₾</span>
                                    </div>
                                  )}
                                </div>
                              </div>
                  ...
                  <Card className="border border-gray-100 hover:border-gray-200 transition-colors">
                    <CardContent className="p-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <h4 className="text-base font-semibold text-gray-900 mb-1">აქტიური მიწოდებები არ არის</h4>
                          <p className="text-sm text-gray-500">შეკვეთის განთავსების შემდეგ შეგიძლიათ თვალი ადევნოთ მიწოდებას</p>
                        </div>
                        <Button 
                          variant="ghost" 
                          size="sm"
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
                ...
                {isEditing ? (
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label className="text-gray-500 text-sm">სრული სახელი</Label>
                        <p>{user.full_name || "არ არის მითითებული"}</p>
                      </div>
                      <div>
                        <Label className="text-gray-500 text-sm">ელ. ფოსტა</Label>
                        <div className="flex items-center">
                      ...
                      <div>
                        <Label className="text-gray-500 text-sm">ტელეფონი</Label>
                        <div className="flex items-center">
                      ...
                      <div>
                        <Label className="text-gray-500 text-sm">ძირითადი მისამართი</Label>
                        <div className="flex items-center">
          ...
          {activeTab === "help" && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
              ...
              <p className="text-gray-600 mb-4">
                თუ თქვენ გაქვთ რაიმე კითხვა ან საჭიროებთ დახმარებას, შეგიძლიათ დაგვიკავშირდეთ:
              </p>
                ...
                <div className="space-y-2">
                  <div className="p-3 bg-gray-50 rounded-lg">
                    ...
                    <p className="text-gray-600 text-sm mt-1">
                      ბალანსის შესავსებად შეგიძლიათ გამოიყენოთ საბანკო ბარათი ან ინტერნეტ ბანკი.
                    </p>
                  </div>
                  <div className="p-3 bg-gray-50 rounded-lg">
                    <p className="font-medium">როგორ შევცვალო მისამართი?</p>
                    <p className="text-gray-600 text-sm mt-1">
                      მისამართების სექციაში შეგიძლიათ დაამატოთ, შეცვალოთ ან წაშალოთ მისამართები.
                    </p>
                  </div>
                </div>
              </div>
          ...
          {activeTab === "orders" && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 text-center">
                ...
                <p className="text-gray-400 text-sm">შეკვეთების ისტორია გამოჩნდება აქ მათი განთავსების შემდეგ</p>
                <Link to="/products">
          ...
          {activeTab === "favorites" && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <p className="text-gray-600">{Array.isArray(favorites) ? favorites.length : 0} პროდუქტი</p>
              </div>
              
              {!Array.isArray(favorites) || favorites.length === 0 ? (
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6 text-center">
                  <Heart className="h-12 w-12 mx-auto text-gray-400 mb-2" />
                  <p className="text-gray-500">ჯერ არ გაქვთ რჩეული პროდუქტები</p>
                  <p className="text-gray-400 text-sm">რჩეულებში დასამატებლად დააჭირეთ გულის ღილაკს პროდუქტის გვერდზე</p>
                  <Link to="/products">
                    <Button className="mt-4">პროდუქტების ნახვა</Button>
                  </Link>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {Array.isArray(favorites) && favorites.map((favorite: any) => (
                    <Card key={favorite.id || favorite.productId} className="overflow-hidden group hover:shadow-lg transition-shadow">
                      ...
                      <div className="p-4">
                        <Link to={favorite.productUrl || `/product/${favorite.productId}`}>
        ...
        <DialogContent className="sm:max-w-md border-0 shadow-xl rounded-xl p-0 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 text-white text-center">
            <div className="flex items-center justify-center mb-2">
              ...
              <DialogTitle className="text-lg font-semibold">ტელეფონის ვერიფიკაცია</DialogTitle>
            </div>
          ...
          <div className="p-6 space-y-4">
            ...
            {!otpSent && (
              <div className="text-center">
                ...
                >
                  {sendOtpMutation.isPending ? 'გაგზავნა...' : 'თავიდან გაგზავნა'}
                </Button>
              </div>
            )}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button 
                ...
                disabled={verificationCode.length !== 4 || verifyOtpMutation.isPending}
