...
import { normalizeGeorgian } from '@/utils/georgian-utils';
...
export function MobileCategoryMenu({
  ...
  const getCategoryIcon = (category: CategoryItem) => {
    ...
    if (name.includes('ბავშვ'))
      return <Baby className="h-5 w-5 text-yellow-600" />;
    if (name.includes('ტანსაცმელ') || name.includes('მოდა'))
      return <Shirt className="h-5 w-5 text-purple-600" />;
    if (name.includes('სახლ') || name.includes('ავეჯ'))
      return <Home className="h-5 w-5 text-green-600" />;
    if (name.includes('ელექტრო') || name.includes('ტექნიკ'))
      return <Smartphone className="h-5 w-5 text-gray-600" />;
    if (name.includes('სპორტ'))
      return <Dumbbell className="h-5 w-5 text-red-600" />;
    if (name.includes('წიგნ') || name.includes('განათლებ'))
      return <BookOpen className="h-5 w-5 text-indigo-600" />;
    if (name.includes('ავტო') || name.includes('მანქან'))
      return <Car className="h-5 w-5 text-orange-600" />;
    if (name.includes('მოგზაურ'))
      return <Plane className="h-5 w-5 text-cyan-600" />;
    if (name.includes('სილამაზ'))
            ...
            {categories.map((category) => (
              ...
              >
                ...
                >
                  ...
                  <span>{normalizeGeorgian(category.Name)}</span>
